<template>
  <section class="section">
    ...
    <div class="container">
      <v-client-table :data="tableData" :columns="columns" url="https://api.github.com/users/matfish2/repos" :options="options"></v-client-table>
    </div>
    </article>
    <div>
    ...
    <!-- <router-link to="/user-list" class="navbar-item">user List</router-link> -->
  </section>
</template>
...
export default {
  ...
  created () {
    var self = this
    db.collection('Users').doc(this.$store.getters.getUser.uid).get().then(function (doc) {
      console.log(doc.data())
      if (doc.data() !== undefined) {
        self.todos = doc.data().todos
      }
    })
  },
  methods: {
    addTodo (title) {
      ...
      db.collection('Users').doc(this.$store.getters.getUser.uid).update({
        todos: this.todos
      })
      // db.collection('Users').doc(this.$store.getters.getUser.uid).get().then(function (doc) {
      //   console.log("fd" + doc.child())
      //   doc.data().push({title: title.title,
